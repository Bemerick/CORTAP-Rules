"""
LOE (Level of Effort) Analysis Script
Uses Anthropic Claude API to analyze FTA sub-areas and estimate review hours
"""
...
def process_fta_data(fta_data: Dict):
    ...
    print(f"\nStarting LOE analysis for {total_sections} sections and {total_sub_areas} sub-areas...")
    print("=" * 80)
    ...
    try:
        for section_idx, section in enumerate(fta_data.get('sections', []), 1):
            ...
            print(f"\n[{section_idx}/{total_sections}] Processing Section: {section_id} - {section_data.get('title')}")
            ...
            for sub_area in section.get('sub_areas', []):
                ...
                print(f"  → Analyzing sub-area {sub_area_id}...", end=' ')

                # Analyze LOE using Claude
                loe_data = analyze_sub_area_loe(sub_area)

                if loe_data:
                    print(f"✓ {loe_data.get('hours')} hrs ({loe_data.get('confidence_level')} confidence)")
                else:
                    print("✗ Failed")
...
def display_summary():
    ...
    try:
        ...
        cursor.execute("""
            ...
            FROM sub_areas
            WHERE loe_hours IS NOT NULL
        """)

        result = cursor.fetchone()

        print("\n" + "=" * 80)
        print("LOE ANALYSIS SUMMARY")
        print("=" * 80)
        print(f"Total Sub-Areas:     {result[0]}")
        print(f"Total Hours:         {result[1]:.2f} hrs")
        print(f"Average Hours:       {result[2]:.2f} hrs per sub-area")
        print(f"Average Confidence:  {result[3]:.1f}%")
        print("=" * 80)

        # Get section summary
        cursor.execute("SELECT * FROM section_loe_summary ORDER BY section_id")
        ...
        for section in sections:
            ...
            if hours:
                print(f"{section_id:15} {title:40} {hours:6.2f} hrs ({confidence:.0f}% conf)")
        print("=" * 80)
...
def main():
    ...
    print("FTA Comprehensive Review - LOE Analysis Tool")
    print("=" * 80)
